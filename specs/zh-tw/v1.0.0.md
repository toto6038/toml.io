...
TOML 的目標是成為一種語意清楚且簡潔明瞭的組態檔格式。
TOML 被設計為可以無歧義地對應到雜湊表 (Hash table)。
TOML 可以容易地在各種語言中剖析為資料結構。
...
* TOML 將大小寫字母視為不同字
* TOML 檔案必須是有效的、以 UTF-8 編碼的 Unicode 文件
* 空白字元為 Tab (0x09) 或 Space (0x20)
* 換行字元為 LF (0x0A) 或 CRLF (0x0D 0x0A)

註解
-------

以「#」號表示該行接下來的內容為註解，唯一的例外是字串內容
...
註解中不允許存在 Tab (U+0000 to U+0008, U+000A to U+001F, U+007F) 以外的控制字元

鍵值對
--------------

鍵值對 (key/value pair) 乃構成 TOML 文件的主要角色

鍵、值分別在等號的左、右邊。
鍵名與值周圍的空白會被忽略。
鍵、等號、值必須在同一行上，但有些值可以被拆為多行。
...
每對鍵值對後須有換行或 EOF 字元（關於例外情況，請參閱[行內表格](#inline-table)）
...
整數是沒有小數部分的數字。正整數前可加正號，負整數前須加負號。
...
前導的數字 0 不允許存在。`-0` 與 `+0` 是合法的且與無正負號的 0 相同。

非負整數的數值可以用十六進位、八進位、二進位系統表示。當使用這三種進位系統時，前導的「`+`」號不可存在，然而前置碼後的前導 0 可以存在。
十六進位數值不區分大小寫。數字之間可用底線分隔，但不可置於前置碼與數值間。
...
# 以 `0o` 開始表示八進位數值
oct1 = 0o01234567
oct2 = 0o755 # 表示 Unix 檔案權限時很方便

# 以 `0b` 開始表示二進位數值
bin1 = 0b11010110
```
任意 64 位元的有號整數（介於 -2^63 至 2^63-1）應被接受且無失真地處理。當整數無法被正確表示時，必須拋出錯誤。

浮點數
-----

浮點數應採用與 IEEE 754 binary64 相同的實作方式。

浮點數的組成為一個整數部分（此部分遵循著與十進位整數相同的規則）後接著一個小數部分及/或指數部分。
若小數部分及指數部分同時存在，則小數部分應先於指數部分。
...
同整數一樣，可以使用底線符號來提升可讀性。每個底線符號兩側須有至少一位數字。
...
TOML 檔案應使用 `.toml` 作為副檔名。

MIME 類型
---------

透過網際網路傳送 TOML 檔案時，適用的 MIME 類型為`application/toml`。
